...
|---|:---:|---|---|
| `IDNA_DIR` | No | repo root (inferred from `idna/config.py` path) | Code root. Used by setup scripts to locate `idna_server.py`, `templates/`, `types/`. |
...
| `Makefile` | `ls` / `clean` helpers; service runs via `uv run idna_server.py` | Yes |
...
There is no `.env` auto-load inside `idna/config.py` — env vars must be set in the shell (or via `export`) before running `uv run idna_server.py`.
