...
{{
  ...
  "axes": [
    ...
    //   Style first:   medium (0=flat icon/sticker → 1=3D render/sculptural), finish (0=line art → 1=filled/solid)
    //   Concept next:  abstraction, tech_feel, energy
    //   Form next:     geometry, weight, symmetry
    //   Color then:    hue (0=cool/blue → 1=warm/red), background (0=light → 1=dark)
    // For logo: DO NOT include type_weight or type_style axes (logos are pure marks, no text)
    // CRITICAL: low/high values must be SHORT — 2 to 4 words max. No synonym lists.
    //   GOOD: "flat icon, clean"  BAD: "flat icon, vector, simple, clean, 2D, minimal, modern"
    {{"name": "axis_name", "low": "2-4 word descriptor", "high": "2-4 word descriptor"}}
  ],
...
// For logo anchor: describe the subject + implied visual style. Do NOT pile up constraints here.
//   e.g. "abstract symbol mark for a modular AI toolkit, geometric, dark background"
